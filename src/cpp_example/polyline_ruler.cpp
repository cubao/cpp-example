...
{
    if (is_wgs84) {
        return utils::lla2enu_cheap(a.transpose(), b).row(0).squaredNorm();
    }
...
{
    if (is_wgs84) {
        return lineDistance(utils::lla2enu_cheap(line), !is_wgs84);
    }
    int N = line.rows();
    if (N < 2) {
        return 0.0;
                     ...
                     bool is_wgs84)
{
    ...
    if (is_wgs84) {
        auto ret = along(utils::lla2enu_cheap(line), dist, !is_wgs84);
        if (ret) {
                                             ...
                                             bool is_wgs84)
{
    if (is_wgs84) {
        ...
        llas.row(1) = a;
        llas.row(2) = b;
        auto enus = utils::lla2enu_cheap(llas);
                           ...
                           const Eigen::Vector3d &p, bool is_wgs84)
{
    ...
    if (is_wgs84) {
        ...
        auto ret = pointOnLine(
            utils::lla2enu_cheap(line, anchor),
            utils::lla2enu_cheap(p.transpose(), anchor).row(0), !is_wgs84);
        std::get<0>(ret) =
                                    ...
                                    bool is_wgs84)
{
    ...
    if (is_wgs84) {
        ...
        auto enus = utils::lla2enu_cheap(start_stop, anchor);
        return utils::enu2lla_cheap(
            lineSlice(enus.row(0), enus.row(1),
                      utils::lla2enu_cheap(line, anchor), !is_wgs84),
            anchor);
    }
                              ...
                              bool is_wgs84)
{
    ...
    if (is_wgs84) {
        return utils::enu2lla_cheap(
            lineSliceAlong(start, stop, utils::lla2enu_cheap(line), !is_wgs84),
            line.row(0));
    }
    double sum = 0.;
                                           ...
                                           bool is_wgs84)
{
    if (is_wgs84) {
        ...
        auto enus = utils::lla2enu_cheap(llas);
        return utils::enu2lla_cheap(
                   interpolate(enus.row(0), enus.row(1), t, !is_wgs84)
                       .transpose(),
                   llas.row(0))
            .row(0);
    }
                         ...
                         tl::optional<Eigen::Vector3d> lla)
{
    ...
    CheapRuler ruler((*lla)[1], CheapRuler::Unit::Meters);
    auto k = ruler.k();
