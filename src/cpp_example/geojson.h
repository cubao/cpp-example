...
const void
parseProperties(const mapbox::feature::property_map &property_map,
                ...
                std::unordered_map<std::string, ColumnMeta> &columnMetas)
{
    for (const auto &kv : property_map) {
        ...
        std::copy(reinterpret_cast<const uint8_t *>(&column_index),
                  reinterpret_cast<const uint8_t *>(&column_index + 1),
...
const void introspectColumnMetas(const feature &f,
                                 std::vector<ColumnMeta> &columnMetas)
{
    ...
    for (auto p : f.properties) {
        ...
        columnMetas.push_back(
            ColumnMeta{static_cast<uint8_t>(type), name, i++});
    }
}

const void writeHeader(const char *name, std::vector<double> *envelope,
                       uint16_t indexNodeSize, GeometryType geometryType,
                       std::vector<ColumnMeta> &columnMetas,
                       ...
                       const std::function<void(uint8_t *, size_t)> &writeData)
{
    ...
    for (auto i = 0; i < columnMetas.size(); i++)
        columns.push_back(CreateColumnDirect(fbb, columnMetas[i].name.c_str(),
                                             (ColumnType)columnMetas[i].type));
    auto pColumns = columns.size() > 0 ? &columns : nullptr;
    auto header = CreateHeaderDirect(fbb, nullptr, envelope, geometryType,
                                     false, false, false, false, pColumns,
                                     featuresCount, indexNodeSize, crs);
    fbb.FinishSizePrefixed(header);
    writeData(fbb.GetBufferPointer(), fbb.GetSize());
}

const std::vector<point> extractPoints(const double *coords, uint32_t length,
                                       uint32_t offset = 0)
{
    ...
    auto points2 = iter::imap([] (auto pair) { return point( pair[0],
    pair[1]);}, pairs); std::vector<point> points { points2.begin(),
    points2.end() }; return points;
    */
}

const multi_line_string fromMultiLineString(const double *coords,
                                            const uint32_t coordsLength,
                                            const Vector<uint32_t> *ends)
{
    if (ends == nullptr || ends->size() < 2)
        return multi_line_string{
            line_string(extractPoints(coords, coordsLength))};
    std::vector<line_string> lineStrings;
    uint32_t offset = 0;
    for (uint32_t i = 0; i < ends->size(); i++) {
        uint32_t end = ends->Get(i) << 1;
        lineStrings.push_back(
            line_string(extractPoints(coords, end - offset, offset)));
        offset = end;
    }
    return multi_line_string(lineStrings);
}

const polygon fromPolygon(const double *coords, const uint32_t coordsLength,
                          const Vector<uint32_t> *ends)
{
    if (ends == nullptr || ends->size() < 2)
    ...
    for (uint32_t i = 0; i < ends->size(); i++) {
        uint32_t end = ends->Get(i) << 1;
        linearRings.push_back(
            linear_ring(extractPoints(coords, end - offset, offset)));
        offset = end;
    }
    return polygon(linearRings);
}

const geometry fromGeometry(const Geometry *geometry,
...
readGeoJsonProperties(const Feature *feature,
                      const std::vector<ColumnMeta> &columnMetas)
{
    ...
    while (offset < size) {
        ...
        case ColumnType::Double:
            value.set<double>(
                *(reinterpret_cast<const double *>(data + offset)));
            offset += sizeof(double);
            break;
        case ColumnType::String: {
            uint32_t len = *(reinterpret_cast<const uint32_t *>(data + offset));
            offset += sizeof(uint32_t);
            value.set<std::string>(std::string(
...
const mapbox::feature::feature<double>
fromFeature(const Feature *feature, const GeometryType geometryType,
            const std::vector<ColumnMeta> &columnMetas)
{
...
const uoffset_t
readFeature(const std::function<void(const void *, const size_t)> &readData,
...
const void
serialize(const std::function<const feature *()> &readFeature,
          const std::function<const void(void *, const size_t)> &writeData,
          const uint64_t featuresCount = 0, const bool createIndex = false)
{
    auto f = readFeature();
    if (f == nullptr)
        throw std::runtime_error(
            "Unable to read a feature (need at least one)");
    ...
    if (!createIndex) {
        ...
        writeHeader(nullptr, nullptr, 0, geometryType, columnMetas,
                    featuresCount, writeData);
        while (f != nullptr) {
    ...
    writeData(magicbytes, sizeof(magicbytes));
    writeHeader(nullptr, pEnvelope, 16, geometryType, columnMetas, items.size(),
                writeData);
...
const void serialize(const feature_collection &fc,
...
const void
deserialize(const std::function<void(const void *, const size_t)> &readData,
            const std::function<void(const feature &)> &writeFeature,
            ...
            const NodeItem *nodeItem = nullptr)
{
    ...
    if (indexNodeSize > 0) {
        if (seekData != nullptr && nodeItem != nullptr) {
            ...
            const auto readNode = [treeOffset, &seekData, &readData](
                                      uint8_t *buf, size_t i, size_t s) {
                seekData(treeOffset + i);
                readData(buf, s);
            };
            const auto result = PackedRTree::streamSearch(
                featuresCount, indexNodeSize, *nodeItem, readNode);
...
const feature_collection deserialize(const void *buf)
{
    ...
    const auto writeFeature = [&fc](const feature &f) { fc.push_back(f); };
...
const feature_collection deserialize(const void *buf, const NodeItem rect)
{
    ...
    const auto writeFeature = [&fc](const feature &f) { fc.push_back(f); };
    const auto seekData = [&offset](size_t newoffset) { offset = newoffset; };
