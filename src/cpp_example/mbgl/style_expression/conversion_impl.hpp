...
/*
   ...
   conforming to the schema defined by the Mapbox Style Specification, to the
   ...
   exists
      ...
      * `eachMember(v, [] (const std::string&, const V&) -> optional<Error>
...
template <typename T> class ConversionTraits;
...
class Convertible
{
  public:
    ...
    friend inline optional<Convertible> objectMember(const Convertible &v,
                                                     const char *name)
    {
    ...
    eachMember(const Convertible &v,
               const std::function<optional<Error>(const std::string &,
    ...
    friend inline optional<Feature> toGeoJSON(const Convertible &v,
                                              Error &error)
    {
  ...
  private:
                                            ...
                                            const Convertible &)> &fn)
    {
        return ConversionTraits<T>::eachMember(
            reinterpret_cast<const T &>(s), [&](const std::string &k, T &&v) {
                return fn(k, Convertible(std::move(v)));
            });
    }

    template <typename T> static VTable *vtableForType()
    {
        ...
        static VTable vtable = {
            ...
            [](const Storage &s, std::size_t i) {
                return Convertible(
                    Traits::arrayMember(reinterpret_cast<const T &>(s), i));
            },
            [](const Storage &s) {
            ...
            [](const Storage &s, const char *key) {
                optional<T> member =
                    Traits::objectMember(reinterpret_cast<const T &>(s), key);
                if (member) {
...
struct ValueFactory<
    T, typename std::enable_if<(!std::is_enum<T>::value &&
                                !is_linear_container<T>::value)>::type>
{
...
template <typename T>
struct ValueFactory<
    T, typename std::enable_if<is_linear_container<T>::value>::type>
{
    static Value make(const T &arg)
    {
        ...
        for (const auto &item : arg) {
            result.emplace_back(
                ValueFactory<std::decay_t<decltype(item)>>::make(item));
        }
        return result;
    }
};

template <typename T> Value makeValue(T &&arg)
{
