...
#define MBGL_DEFINE_ENUM(T, values...)                                         \
                                                                               \
    static const constexpr std::pair<const T, const char *> T##_names[] =      \
        values;                                                                \
                                                                               \
    template <> const char *Enum<T>::toString(T t)                             \
    {                                                                          \
        ...
        return it->second;                                                     \
                                                                               ...
                                                                               \
    template <> optional<T> Enum<T>::toEnum(const std::string &s)              \
    {                                                                          \
        auto it = std::find_if(std::begin(T##_names), std::end(T##_names),     \
