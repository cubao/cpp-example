...
{
    return type == type::String || type.is<type::Array>() ||
           type == type::Null || type == type::Value;
}
...
EvaluationResult In::evaluate(const EvaluationContext &params) const
{
    ...
    if (!isSearchableRuntimeType(evaluatedHaystackType)) {
        return EvaluationError{"Expected second argument to be of type array "
                               "or string, but found " +
    ...
    if (evaluatedHaystackType == type::String) {
        ...
        return EvaluationResult(
            std::find(haystackArray.begin(), haystackArray.end(),
...
ParseResult In::parse(const Convertible &value, ParsingContext &ctx)
{
    ...
    if (length != 3) {
        ctx.error("Expected 2 arguments, but found " +
    ...
    if (!isSearchableType(haystackType)) {
        ctx.error("Expected second argument to be of type array or string, but "
                  "found " +
