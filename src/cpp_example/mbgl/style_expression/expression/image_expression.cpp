...
ParseResult ImageExpression::parse(const Convertible &value,
                                   ParsingContext &ctx)
{
    ...
    return ParseResult(
        std::make_unique<ImageExpression>(std::move(*imageIDExpr)));
}

void ImageExpression::eachChild(
...
EvaluationResult ImageExpression::evaluate(const EvaluationContext &ctx) const
{
    ...
    if (!evaluatedImageID) {
        return EvaluationError(
            {"Could not evaluate ID for 'image' expression."});
    }

    bool available =
        ctx.availableImages && ctx.availableImages->count(*evaluatedImageID);
