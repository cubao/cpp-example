...
ParseResult Let::parse(const Convertible &value, ParsingContext &ctx)
{
    ...
    if (length < 4) {
        ctx.error("Expected at least 3 arguments, but found " +
    ...
    for (std::size_t i = 1; i < length - 1; i += 2) {
        ...
        if (!name) {
            ctx.error("Expected string, but found " +
                          getJSONType(arrayMember(value, i)) + " instead.",
                      i);
            return ParseResult();
        }
        ...
        if (!isValidName) {
            ctx.error("Variable names must contain only alphanumeric "
                      "characters or '_'.",
                      1);
            return ParseResult();
        }
    ...
    ParseResult result_ = ctx.parse(arrayMember(value, length - 1), length - 1,
                                    ctx.getExpected(), bindings_);
    if (!result_) {
        return ParseResult();
    }

    return ParseResult(
        std::make_unique<Let>(std::move(bindings_), std::move(*result_)));
}
...
std::vector<optional<Value>> Var::possibleOutputs() const { return {nullopt}; }
