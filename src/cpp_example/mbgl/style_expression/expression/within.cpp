...
Point<int64_t> latLonToTileCoodinates(const Point<double> &point,
                                      const mbgl::CanonicalTileID &canonical)
{
    ...
    auto y =
        (util::LONGITUDE_MAX -
         (std::log(std::tan(point.y * M_PI / util::DEGREES_MAX + M_PI / 4.0)) *
          util::RAD2DEG)) *
        size / util::DEGREES_MAX;

    Point<int64_t> p;
    p.x = (util::clamp<int64_t>(x, std::numeric_limits<int64_t>::min(),
                                std::numeric_limits<int64_t>::max()));
    p.y = (util::clamp<int64_t>(y, std::numeric_limits<int64_t>::min(),
                                std::numeric_limits<int64_t>::max()));
...
MultiPolygon<int64_t>
getTilePolygons(const Feature::geometry_type &polygonGeoSet,
                const mbgl::CanonicalTileID &canonical, WithinBBox &bbox)
{
...
void updatePoint(Point<int64_t> &p, WithinBBox &bbox,
                 const WithinBBox &polyBBox, const int64_t worldSize)
{
    if (p.x < polyBBox[0] || p.x > polyBBox[2]) {
        const auto getShift = [](Point<int64_t> &point,
                                 const int64_t polygonSide,
        ...
        if (shift == 0)
            shift = getShift(p, polyBBox[2], worldSize);
        p.x += shift;
    }
    updateBBox(bbox, p);
}
...
MultiPoint<int64_t> getTilePoints(const GeometryCoordinates &points,
                                  const mbgl::CanonicalTileID &canonical,
                                  WithinBBox &bbox, const WithinBBox &polyBBox)
{
                           ...
                           const Feature::geometry_type &polygonGeoSet)
{
    ...
    case FeatureType::Point: {
        ...
        MultiPoint<int64_t> points =
            getTilePoints(geometries.at(0), canonical, pointBBox, polyBBox);
        if (!boxWithinBox(pointBBox, polyBBox))
            return false;

        return std::all_of(points.begin(), points.end(),
                           [&polygons](const auto &p) {
    ...
    case FeatureType::LineString: {
        ...
        MultiLineString<int64_t> multiLineString =
            getTileLines(geometries, canonical, lineBBox, polyBBox);
        if (!boxWithinBox(lineBBox, polyBBox))
            return false;

        return std::all_of(multiLineString.begin(), multiLineString.end(),
                           [&polygons](const auto &line) {
                               return lineStringWithinPolygons<int64_t>(
                                   line, polygons);
                           });
    }
    default:
...
mbgl::optional<mbgl::GeoJSON>
parseValue(const mbgl::style::conversion::Convertible &value_,
           mbgl::style::expression::ParsingContext &ctx)
{
    ...
    ctx.error("'within' expression requires valid geojson source that contains "
              "polygon geometry type.");
    return nullopt;
}

mbgl::optional<Feature::geometry_type>
getPolygonInfo(const Feature &polyFeature,
               mbgl::style::expression::ParsingContext &ctx)
{
...
EvaluationResult Within::evaluate(const EvaluationContext &params) const
{
    ...
    if (geometryType == FeatureType::Point ||
        ...
        return featureWithinPolygons(*params.feature, *params.canonical,
                                     geometries);
    }
    mbgl::Log::Warning(mbgl::Event::General,
                       "within expression currently only support "
                       "Point/LineString geometry type.");
...
ParseResult Within::parse(const Convertible &value, ParsingContext &ctx)
{
    if (isArray(value)) {
        ...
        if (arrayLength(value) != 2) {
            ctx.error("'within' expression requires exactly one argument, but "
                      "found " +
        ...
        return parsedValue->match(
            [&parsedValue,
             &ctx](const mapbox::geometry::geometry<double> &geometrySet) {
                if (auto ret =
                        getPolygonInfo(mbgl::Feature(geometrySet), ctx)) {
                    return ParseResult(std::make_unique<Within>(
                        *parsedValue, std::move(*ret)));
                }
                return ParseResult();
            },
            [&parsedValue,
             &ctx](const mapbox::feature::feature<double> &feature) {
            ...
            [&parsedValue, &ctx](
                ...
                for (const auto &feature : features) {
                    if (auto ret =
                            getPolygonInfo(mbgl::Feature(feature), ctx)) {
                        return ParseResult(std::make_unique<Within>(
                            *parsedValue, std::move(*ret)));
                    }
                }
            ...
            [&ctx](const auto &) {
                ctx.error("'within' expression requires valid geojson source "
                          "that contains polygon geometry type.");
                return ParseResult();
            });
    }
    ctx.error(
        "'within' expression needs to be an array with exactly one argument.");
    return ParseResult();
}
...
mbgl::Value Within::serialize() const
{
    ...
    if (value.IsObject()) {
        ...
        mbgl::Log::Error(mbgl::Event::General,
                         "Failed to serialize 'within' expression, converted "
...
std::string Within::getOperator() const { return "within"; }
