...
struct Signature<R(const Varargs<T> &)> : SignatureBase
{
                           ...
                           const Args &args) const override
    {
        ...
        for (const auto &arg : args) {
            const EvaluationResult evaluatedArg =
                arg->evaluate(evaluationParameters);
            if (!evaluatedArg)
                return evaluatedArg.error();
            evaluated.push_back(
                *fromExpressionValue<std::decay_t<T>>(*evaluatedArg));
        }
...
optional<double> featurePropertyAsDouble(const EvaluationContext &params,
                                         const std::string &key)
{
    assert(params.feature);
    auto property = params.feature->getValue(key);
    ...
    return property->match([](double value) { return value; },
                           [](uint64_t value) -> optional<double> {
                               return {static_cast<double>(value)};
                           },
                           [](int64_t value) -> optional<double> {
                               return {static_cast<double>(value)};
                           },
                           [](const auto &) -> optional<double> { return {}; });
};

optional<std::string> featurePropertyAsString(const EvaluationContext &params,
                                              const std::string &key)
{
    assert(params.feature);
    auto property = params.feature->getValue(key);
    ...
    static auto signature = detail::makeSignature(
        ...
        [](const EvaluationContext &params,
           const std::string &key) -> Result<bool> {
            if (!params.feature) {
                return EvaluationError{"Feature data is unavailable in the "
    ...
    static auto signature = detail::makeSignature(
        ...
        [](const std::string &key,
           const std::unordered_map<std::string, Value> &object)
            -> Result<bool> { return object.find(key) != object.end(); });
    return signature;
}
    ...
    static auto signature = detail::makeSignature(
        ...
        [](const EvaluationContext &params,
           const Varargs<Value> &varargs) -> Result<bool> {
            ...
            return value ? std::find(varargs.begin() + 1, varargs.end(),
                                     *value) != varargs.end()
                         : false;
        });
    return signature;
    ...
    for (auto it = definitions.first; it != definitions.second; ++it) {
        ...
        if (signature->params.is<std::vector<type::Type>>()) {
            ...
            const type::Type &paramType =
                signature->params.get<VarargsType>().type;
            for (std::size_t i = 0; i < args.size(); i++) {
                const std::unique_ptr<Expression> &arg = args[i];
                optional<std::string> err =
        ...
        if (signatureContext.getErrors().empty()) {
            return ParseResult(std::make_unique<CompoundExpression>(
                                    ...
                                    ParsingContext &ctx)
{
    ...
    const auto definitions =
        compoundExpressionRegistry.equal_range(name.c_str());
    ...
    for (auto it = definitions.first; it != definitions.second; ++it) {
        ...
        if (signature->params.is<VarargsType>() ||
            signature->params.get<std::vector<type::Type>>().size() ==
                length - 1) {
    ...
    // Parse the arguments again without expected types just for the error
    // message
    ctx.clearErrors();
    std::vector<std::unique_ptr<Expression>> args;
    args.reserve(length - 1);
...
optional<std::size_t> CompoundExpression::getParameterCount() const
{
    return signature.params.match(
        [&](const VarargsType &) -> optional<std::size_t> { return {}; },
        [&](const std::vector<type::Type> &p) -> optional<std::size_t> {
            return p.size();
        });
}
