...
template <class T, class Enable = void> struct ValueConverter
{
...
template <> struct ValueConverter<mbgl::Value>
{
...
template <typename T, std::size_t N> struct ValueConverter<std::array<T, N>>
{
...
template <typename T> Value toExpressionValue(const T &value)
{
    return ValueConverter<T>::toExpressionValue(value);
}

template <typename T> optional<T> fromExpressionValue(const Value &value)
{
...
fromExpressionValues(const std::vector<optional<Value>> &values)
{
