...
EvaluationResult At::evaluate(const EvaluationContext &params) const
{
    ...
    if (i != std::floor(i)) {
        return EvaluationError{"Array index must be an integer, but found " +
                               util::toString(i) + " instead."};
    }
...
ParseResult At::parse(const Convertible &value, ParsingContext &ctx)
{
    ...
    type::Type inputType =
        type::Array(ctx.getExpected() ? *ctx.getExpected() : type::Value);
    ...
    return ParseResult(
        std::make_unique<At>(std::move(*index), std::move(*input)));
}
