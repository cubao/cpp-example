...
{
    return {"Expected " + toString(expected) + " but found " + toString(t) +
            " instead."};
}
...
optional<std::string> checkSubtype(const Type &expected, const Type &t)
{
    ...
    optional<std::string> result = expected.match(
        [&](const Array &expectedArray) -> optional<std::string> {
            ...
            if (!actualArray.N || *actualArray.N != 0 ||
                actualArray.itemType != type::Value) {
                const auto err =
                    checkSubtype(expectedArray.itemType, actualArray.itemType);
                if (err) {
                    return {errorMessage(expected, t)};
                }
            }
