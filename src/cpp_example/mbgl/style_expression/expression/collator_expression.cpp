...
ParseResult CollatorExpression::parse(const Convertible &value,
                                      ParsingContext &ctx)
{
    ...
    const optional<Convertible> caseSensitiveOption =
        objectMember(options, "case-sensitive");
    ...
    const optional<Convertible> diacriticSensitiveOption =
        objectMember(options, "diacritic-sensitive");
    ParseResult diacriticSensitive;
    if (diacriticSensitiveOption) {
        diacriticSensitive =
            ctx.parse(*diacriticSensitiveOption, 1, {type::Boolean});
    } else {
    ...
    return ParseResult(std::make_unique<CollatorExpression>(
        std::move(*caseSensitive), std::move(*diacriticSensitive),
        std::move(locale)));
}

void CollatorExpression::eachChild(
...
EvaluationResult
CollatorExpression::evaluate(const EvaluationContext &params) const
{
