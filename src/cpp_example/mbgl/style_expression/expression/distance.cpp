...
std::pair<mbgl::optional<IndexRange>, mbgl::optional<IndexRange>>
...
bool isPolygonValid(const mapbox::geometry::polygon<double> &polygon)
{
    if (polygon.empty()) {
        mbgl::Log::Error(mbgl::Event::Style,
                         "Invalid Polygon with empty rings");
        return false;
    }
    for (const auto &ring : polygon) {
        if (ring.size() < 3) {
                              ...
                              mapbox::cheap_ruler::CheapRuler &ruler)
{
    ...
    for (const auto &ring : polygon) {
        if (ring.front() != ring.back()) {
            dist = std::min(
                dist, pointToLineDistance(point,
        ...
        for (const auto &ring : polygon) {
            for (std::size_t j = 0, len = ring.size(), k = len - 1; j < len;
                ...
                if (segmentIntersectSegment(p1, p2, q1, q2))
                    return 0.0;
                dist = std::min(
                    dist, segmentToSegmentDistance(p1, p2, q1, q2, ruler));
            }
        }
    }
    return dist;
           ...
           const mapbox::geometry::polygon<double> &poly2) {
            for (const auto &ring : poly1) {
                for (std::size_t i = 0; i <= ring.size() - 1; ++i) {
                    if (pointWithinPolygon(ring[i], poly2,
                                           true /*trueOnBoundary*/))
                        return true;
                }
            }
    ...
    for (const auto &ring1 : polygon1) {
        for (std::size_t i = 0, len1 = ring1.size(), l = len1 - 1; i < len1;
            ...
            for (const auto &ring2 : polygon2) {
                for (std::size_t j = 0, len2 = ring2.size(), k = len2 - 1;
                     j < len2; k = j++) {
                        ...
                        double currentMiniDist = InfiniteDistance)
{
    ...
    while (!distQueue.empty()) {
        ...
        if (getRangeSize(range) <= MinLinePointsSize) {
            ...
            for (std::size_t i = range.first; i <= range.second; ++i) {
                auto tempDist =
                    pointToPolygonDistance(points[i], polygon, ruler);
            ...
            const auto updateQueue = [&distQueue, &miniDist, &ruler, &points,
                                      &polyBBox](
                                         mbgl::optional<IndexRange> &rangeA) {
                if (!rangeA)
                    return;
                auto tempDist = bboxToBBoxDistance(getBBox(points, *rangeA),
                                                   polyBBox, ruler);
                // Insert new pair to the queue if the bbox distance is less
                // than miniDist, The pair with biggest distance will be at the
                // top
                if (tempDist < miniDist)
                    distQueue.push(std::make_tuple(tempDist, std::move(*rangeA),
                                                   IndexRange(0, 0)));
            };
...
{
    auto miniDist =
        std::min(ruler.distance(line1[0], line2[0]), currentMiniDist);
    if (miniDist == 0.0)
        return miniDist;
    DistQueue distQueue;
    ...
    while (!distQueue.empty()) {
        ...
        if (getRangeSize(rangeA) <= MinLinePointsSize &&
            ...
            auto tempDist =
                lineToLineDistance(line1, rangeA, line2, rangeB, ruler);
            if (std::isnan(tempDist))
                return tempDist;
            miniDist = std::min(miniDist, tempDist);
            if (miniDist == 0.0)
            ...
            const auto updateQueue = [&distQueue, &miniDist, &ruler, &line1,
                                      &line2](
                                         mbgl::optional<IndexRange> &range1,
                                         mbgl::optional<IndexRange> &range2) {
                if (!range1 || !range2)
                    return;
                auto tempDist = bboxToBBoxDistance(
                    getBBox(line1, *range1), getBBox(line2, *range2), ruler);
                // Insert new pair to the queue if the bbox distance is less
                // than miniDist, The pair with biggest distance will be at the
                // top
                if (tempDist < miniDist)
                       ...
                       mapbox::cheap_ruler::CheapRuler &ruler)
{
    ...
    while (!distQueue.empty()) {
        ...
        if (getRangeSize(rangeA) <= MinPointsSize &&
            ...
            auto tempDist = pointsToPointsDistance(pointSet1, rangeA, pointSet2,
                                                   rangeB, ruler);
            if (std::isnan(tempDist))
                return tempDist;
            miniDist = std::min(miniDist, tempDist);
            ...
            const auto updateQueue = [&distQueue, &miniDist, &ruler, &pointSet1,
                ...
                if (!range1 || !range2)
                    return;
                auto tempDist =
                    bboxToBBoxDistance(getBBox(pointSet1, *range1),
...
{
    auto miniDist =
        std::min(currentMiniDist, ruler.distance(points[0], line[0]));
    if (miniDist == 0.0)
        return miniDist;
    DistQueue distQueue;
    distQueue.push(std::forward_as_tuple(0, IndexRange(0, points.size() - 1),
    ...
    while (!distQueue.empty()) {
        ...
        if (getRangeSize(rangeA) <= MinPointsSize &&
            ...
            for (std::size_t i = rangeA.first; i <= rangeA.second; ++i) {
                miniDist = std::min(
                    miniDist, pointToLineDistance(points[i], subLine, ruler));
                if (miniDist == 0.0)
                    return 0.0;
            }
        } else {
            ...
            const auto updateQueue = [&distQueue, &miniDist, &ruler, &points,
                ...
                if (!range1 || !range2)
                    return;
                auto tempDist = bboxToBBoxDistance(
                    getBBox(points, *range1), getBBox(line, *range2), ruler);
                // Insert new pair to the queue if the bbox distance is less
                // than miniDist, The pair with biggest distance will be at the
                // top
                if (tempDist < miniDist)
                         ...
                         const Feature::geometry_type &geoSet)
{
    ...
    return geoSet.match(
        [&points, &ruler](const mapbox::geometry::point<double> &p) {
            return pointsToPointsDistance(
                mapbox::geometry::multi_point<double>{p}, points, ruler);
        },
        [&points,
         &ruler](const mapbox::geometry::multi_point<double> &points1) {
        ...
        [&points, &ruler](
            ...
            for (const auto &polygon : polygons) {
                if (!isPolygonValid(polygon))
                    return InvalidDistance;
                auto tempDist =
                    pointsToPolygonDistance(points, polygon, ruler, dist);
                if (std::isnan(tempDist))
                    return tempDist;
                              ...
                              const Feature::geometry_type &geoSet)
{
    ...
    return geoSet.match(
        [&line, &ruler](const mapbox::geometry::point<double> &p) {
            return pointsToLineDistance(
                mapbox::geometry::multi_point<double>{p}, line, ruler);
        },
        ...
        [&line, &ruler](
            ...
            for (const auto &polygon : polygons) {
                if (!isPolygonValid(polygon))
                    return InvalidDistance;
                auto tempDist =
                    lineToPolygonDistance(line, polygon, ruler, dist);
                if (std::isnan(tempDist))
                    return tempDist;
                dist = std::min(dist, tempDist);
                if (dist == 0.0)
                          ...
                          const Feature::geometry_type &geoSet)
{
    ...
    return geoSet.match(
        ...
        [&polygon,
         &ruler](const mapbox::geometry::multi_point<double> &points) {
            if (!isMultiPointValid(points))
                return InvalidDistance;
            return pointsToPolygonDistance(points, polygon, ruler);
        },
                         ...
                         const Feature::geometry_type &geoSet)
{
    return convertGeometry(feature, canonical)
        .match(
            ...
            [&geoSet](
                const mapbox::geometry::polygon<double> &polygon) -> double {
...
optional<GeoJSON> parseValue(const style::conversion::Convertible &value,
                             style::expression::ParsingContext &ctx)
{
    if (isArray(value)) {
        ...
        if (length != 2) {
            ctx.error(
                "'distance' expression requires one argument, but found " +
...
optional<Feature::geometry_type>
...
EvaluationResult Distance::evaluate(const EvaluationContext &params) const
{
    if (!params.feature || !params.canonical) {
        return EvaluationError{"distance expression requirs valid feature and "
        ...
        [&parsedValue,
         &ctx](const mapbox::feature::feature_collection<double> &features) {
            for (const auto &feature : features) {
                if (auto ret = getGeometry(mbgl::Feature(feature), ctx)) {
                    return ParseResult(std::make_unique<Distance>(
                        *parsedValue, std::move(*ret)));
                }
            }
            return ParseResult();
        },
        [&ctx](const auto &) {
            ctx.error("'distance' expression requires valid geojson that "
                      "contains Point/LineString/Polygon geometries.");
            return ParseResult();
        });
