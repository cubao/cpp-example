...
{
  public:
    Interpolate(const type::Type &type_, Interpolator interpolator_,
    ...
    void
    eachStop(const std::function<void(double, const Expression &)> &visit) const
    {
        for (const auto &stop : stops) {
    ...
    // Return the smallest range of stops that covers the interval [lower,
    // upper]
    ...
    double interpolationFactor(const Range<double> &inputLevels,
                               const double inputValue) const
    {
