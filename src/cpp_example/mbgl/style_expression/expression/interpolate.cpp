...
ParseResult parseInterpolate(const Convertible &value, ParsingContext &ctx)
{
    ...
    if (interpName && *interpName == "linear") {
        ...
        if (!base) {
            ctx.error("Exponential interpolation requires a numeric base.", 1,
                      1);
            return ParseResult();
        }
        ...
        if (!x1 || !y1 || !x2 || !y2 || *x1 < 0 || *x1 > 1 || *y1 < 0 ||
            *y1 > 1 || *x2 < 0 || *x2 > 1 || *y2 < 0 || *y2 > 1) {
            ctx.error("Cubic bezier interpolation requires four numeric "
                      "arguments with values between 0 and 1.",
                      1);
            return ParseResult();
        }
    ...
    if (!interpolator) {
        ctx.error("Unknown interpolation type " +
