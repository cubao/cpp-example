...
template <class T> class FormatSectionOverride final : public Expression
{
  public:
    ...
    void
    eachChild(const std::function<void(const Expression &)> &fn) const final
    {
        defaultValue.match(
            [&fn](const style::PropertyExpression<T> &e) {
                fn(e.getExpression());
            },
            [](const T &) {});
    }
            ...
            return defaultValue.match(
                [other](const style::PropertyExpression<T> &thisExpr) {
                    return other->defaultValue.match(
                        [&thisExpr](
                            const style::PropertyExpression<T> &otherExpr) {
