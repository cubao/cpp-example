...
template <typename T> optional<Value> checkNumber(T n)
{
...
ParseResult Literal::parse(const Convertible &value, ParsingContext &ctx)
{
    if (isObject(value)) {
        ...
        if (arrayLength(value) != 2) {
            ctx.error("'literal' expression requires exactly one argument, but "
                      "found " +
                      util::toString(arrayLength(value) - 1) + " instead.");
            return ParseResult();
        }
        const optional<Value> parsedValue =
            parseValue(arrayMember(value, 1), ctx);
        if (!parsedValue) {
            return ParseResult();
        }
...
mbgl::Value Literal::serialize() const
{
    if (getType().is<type::Array>() || getType().is<type::ObjectType>()) {
        return std::vector<mbgl::Value>{
            {getOperator(), *fromExpressionValue<mbgl::Value>(value)}};
    } else {
