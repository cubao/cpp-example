...
Step::Step(const type::Type &type_, std::unique_ptr<Expression> input_,
           std::map<double, std::unique_ptr<Expression>> stops_)
    : Expression(Kind::Step, type_), input(std::move(input_)),
...
void Step::eachStop(
    const std::function<void(double, const Expression &)> &visit) const
{
    for (const auto &stop : stops) {
...
bool Step::operator==(const Expression &e) const
{
    if (e.getKind() == Kind::Step) {
        ...
        return *input == *(rhs->input) &&
               Expression::childrenEqual(stops, rhs->stops);
    }
    return false;
}
...
Range<float> Step::getCoveringStops(const double lower,
                        ...
                        ParsingContext &ctx)
{
    ...
    stops.emplace(-std::numeric_limits<double>::infinity(),
                  std::move(*firstOutput));
    ...
    return ParseResult(std::make_unique<Step>(*outputType, std::move(*input),
                                              std::move(stops)));
}
